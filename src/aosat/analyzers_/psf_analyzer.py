...
class psf_analyzer():
    """Analyzes the PSF properties like shape, Strehl, Tip-Tilt variation.

    Parameters
    ----------
    sd : dict
        Setup dictionary generated by :meth:`aosat.analyze.setup`.
    ...
    psf : 2D ndarray (float)
        Time-averaged PSF.
    strehl : float
        Strehl ratio of PSF derived from peak intensity.
    sr_wf : float
        Strehl ratio of PSF derived from residual wave fronts.
    ttx : 1D ndarray (float) of length n_frames
        Global tip for each frame from Gauss-fitted PSF location (mas).
    tty : 1D ndarray (float) of length n_frames
        Global tilt for each frame from Gauss-fitted PSF location (mas).
    ttilt : 1D ndarray (float) of length n_frames
        Global excursion from centre, determined from wavefront (mas)
    ttjit : float
        rms of ttilt
    ttq90 : float
        90% quantile of ttilt
    ttjit_psf : float
        rms of sqrt(ttx**2+tty**2)
    ttq90_psf : float
        90% quantile of sqrt(ttx**2+tty**2)
